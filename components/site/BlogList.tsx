...
function BlogList({ posts, isHidden, title, categories }: Props) {
	...
	const filteredPosts =
		category?.name === 'all' || isHidden || category === null
			? posts
			: posts.filter((post) =>
					post.categories.find((cate) => cate.title === category?.name),
			  );
	...
	return (
		<div>
			...
			<div
				className={`grid grid-cols-1 gap-10 pb-24 ${
					filteredPosts?.length > 0 ? 'md:grid-cols-2' : 'md:grid-cols-1'
				} gap-y-16`}
			>
